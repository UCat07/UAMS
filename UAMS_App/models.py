...
class AidApplication(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.application_status == 'pending':
            ...
            Notification.objects.create(
                recipient=self.user,
                message=f"Your application {self.application_id} had been rejected by admin. Please track application status for further information.",
                related_aid_application=self
            )
        elif self.application_status == 'accepted':
            Notification.objects.create(
                recipient=self.user,
                message=f"Your application {self.application_id} had been accepted by the admin, please wait for further disbursement.",
                related_aid_application=self
            )
...
class Notification(models.Model):
    ...
    related_aid_application = models.ForeignKey(AidApplication, on_delete=models.CASCADE, blank=True, null=True)
    related_feedback = models.ForeignKey(Feedback, on_delete=models.CASCADE, blank=True, null=True)
