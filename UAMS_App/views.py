...
from .forms import DisburseFundForm  # Assume this form is created to handle amount and duration
...
def manage_notification(request):
    if request.method == "POST":
        # Get the selected notification method from the form
        notification_method = request.POST.get('notification_method')

        # Save the user's preference (this will depend on your user model)
        request.user.notification_method = notification_method
        request.user.save()

        # Redirect to a confirmation page or back to the dashboard
        return redirect('dashboard')  # Or wherever you'd like to redirect after saving

    return render(request, 'UAMS_App/manage_notification.html')
