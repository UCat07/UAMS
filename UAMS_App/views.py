...
def select_ensure_fund(request):
    ...
    # Get the status filter from the GET request, default to empty (no filter)
    status_filter = request.GET.get('status', '')

    if status_filter in ['disbursed', 'finished', 'cancelled']:
        funds = funds.filter(status=status_filter)

    statuses = ['disbursed', 'finished', 'cancelled']  # Predefined statuses for the dropdown

    return render(request, 'UAMS_App/select_ensure_fund.html', {
        'funds': funds,
        'statuses': statuses,
        'selected_status': status_filter  # Pass the selected status for the dropdown
    })
